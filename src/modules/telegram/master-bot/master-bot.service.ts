...
export class MasterBotService {
  ...
  private loadConfig(): void {
    this.config = {
      ...
      adminIds: (this.configService.get<string>('MASTER_ADMIN_IDS', '')).split(',').filter(id => id.trim()),
      enabledFeatures: {
        autoApproval: this.configService.get<boolean>('MASTER_AUTO_APPROVAL', false),
        userManagement: this.configService.get<boolean>('MASTER_USER_MANAGEMENT', true),
        groupManagement: this.configService.get<boolean>('MASTER_GROUP_MANAGEMENT', true),
        statistics: this.configService.get<boolean>('MASTER_STATISTICS', true),
      }
    };

    this.logger.log(`Мастер-бот настроен. Администраторов: ${this.config.adminIds.length}`);
  }
  ...
  async startMasterBot(): Promise<boolean> {
    ...
    try {
      ...
      await this.notificationService.notifyAdmins('🤖 Мастер-бот запущен и готов к работе');
      return true;
    } catch (error) {
  ...
  async stopMasterBot(): Promise<boolean> {
    if (!this.isRunning || !this.masterBot) {
      this.logger.warn('Мастер-бот не запущен');
      return true;
    }

    try {
      await this.notificationService.notifyAdmins('🔴 Мастер-бот останавливается...');
  ...
  async emergencyStop(): Promise<void> {
    ...
    await this.notificationService.notifyAdmins('🚨 ЭКСТРЕННАЯ ОСТАНОВКА МАСТЕР-БОТА');
