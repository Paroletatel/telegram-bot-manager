...
// Строгий дискриминируемый тип состояния пользователя в мастере ботов
type UserState =
  | { kind: 'waitingForToken' }
  | { kind: 'waitingForBotName'; botToken: string }
  | { kind: 'awaiting_token' }
  | { kind: 'awaiting_name'; token: string };
...
export class MasterBotService {
  ...
  constructor(
    ...
    if (!token) {
      throw new Error('MASTER_BOT_TOKEN is not defined in environment variables');
    }
  ...
  private setupBotManagement(): void {
    ...
    this.bot.onText(/\/createbot/, async (msg: Message) => {
      ...
      this.userStates.set(msg.from.id, { kind: 'waitingForToken' });
    });
    ...
    this.bot.onText(/\/mybots/, async (msg: Message) => {
      ...
      try {
        ...
        if (bots.length === 0) {
          await this.bot.sendMessage(
            msg.chat.id,
            'У вас пока нет созданных ботов. Используйте /createbot для создания нового бота.',
          );
          return;
        }

        const botList = bots
          .map((bot) => `${bot.name} (${bot.isActive ? '✅' : '❌'}) - /toggle_${bot.id}`)
          .join('\n');
        ...
        await this.bot.sendMessage(
          msg.chat.id,
          'Произошла ошибка при получении списка ботов. Пожалуйста, попробуйте позже.',
        );
      }
    });
    ...
    this.bot.on('message', async (msg: Message) => {
      ...
      if (userState?.kind === 'waitingForToken') {
        try {
          ...
          if (!token.match(/^\d+:[-a-zA-Z0-9_]+$/)) {
            await this.bot.sendMessage(
              msg.chat.id,
              'Неверный формат токена. Пожалуйста, попробуйте еще раз.',
            );
            return;
          }

          // Запрашиваем имя бота
          await this.bot.sendMessage(msg.chat.id, 'Теперь введите имя для вашего бота:', {
            reply_markup: { force_reply: true },
          });

          // Обновляем состояние пользователя
          this.userStates.set(userId, {
            kind: 'waitingForBotName',
            botToken: token,
          });
        } catch (error) {
      ...
      else if (userState?.kind === 'waitingForBotName') {
        try {
          ...
          await this.bot.sendMessage(
            msg.chat.id,
            'Произошла ошибка при создании бота. Пожалуйста, попробуйте еще раз.',
          );
        } finally {
    ...
    this.bot.onText(
      ...
      async (msg: Message, match: RegExpExecArray | null) => {
        ...
        try {
          ...
          if (newStatus) {
            ...
            try {
              ...
              if (isStarted) {
                ...
                await this.bot.sendMessage(msg.chat.id, `Бот "${bot.name}" успешно включен! ✅`);
              } else {
              ...
              const errorMessage = error instanceof Error ? error.message : 'Неизвестная ошибка';
              this.logger.error(
                `Критическая ошибка при включении бота ${bot.name}: ${errorMessage}`,
              );
            ...
            try {
              ...
              if (isStopped) {
                ...
                await this.bot.sendMessage(msg.chat.id, `Бот "${bot.name}" успешно выключен. ❌`);
              } else {
          ...
          await this.bot.sendMessage(
            msg.chat.id,
            'Произошла ошибка при переключении бота. Пожалуйста, попробуйте позже.',
          );
        }
      },
    );
  ...
  private setupCommands(): void {
    ...
    this.bot.onText(/\/register/, (msg: Message) => {
      if (!msg.from) {
        this.bot.sendMessage(msg.chat.id, 'Не удалось определить отправителя сообщения');
        return;
      }
      this.userStates.set(msg.from.id, { kind: 'awaiting_token' });
    ...
    this.bot.onText(/\/list/, async (msg: Message) => {
      try {
        const bots = await this.telegramBotModel.findAll();
        const message = bots
          .map((bot) => `${bot.name} - ${bot.isActive ? '✅' : '❌'} (${bot.id})`)
          .join('\n');
    ...
    this.bot.on('text', async (msg: Message) => {
      ...
      try {
        if (state.kind === 'awaiting_token') {
          const token = msg.text;
          this.userStates.set(userId, { kind: 'awaiting_name', token });
          this.bot.sendMessage(msg.chat.id, 'Введите имя бота:');
        } else if (state.kind === 'awaiting_name') {
          const name = msg.text;
