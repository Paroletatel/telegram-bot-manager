...
export class MasterCommandService {
  ...
  setupCommandHandlers(bot: TelegramBot, config: MasterBotConfig): void {
    ...
    bot.onText(/\/pending/, (msg) =>
      this.handlePendingCommand(bot, msg, config)
    );
    bot.onText(/\/approve (.+)/, (msg, match) =>
      this.handleApproveCommand(bot, msg, match, config)
    );
    bot.onText(/\/reject (.+)/, (msg, match) =>
      this.handleRejectCommand(bot, msg, match, config)
    );
...
**Статус:** 🟢 Активен
  ...
  private async handleHealthCommand(
    ...
    try {
      ...
      await bot.sendMessage(msg.chat.id, healthReport, {
        parse_mode: "Markdown",
      });
    } catch (error) {
      ...
      await bot.sendMessage(
        msg.chat.id,
        "❌ Ошибка получения отчета о здоровье"
      );
    }
  }
  ...
  private async handlePendingCommand(
    ...
    try {
      await this.userManagementService.showPendingRegistrations(
        bot,
        msg.chat.id
      );
    } catch (error) {
  ...
  private async handleApproveCommand(
    ...
    const result = await this.userManagementService.approveUser(
      chatId,
  ...
  private async handleRejectCommand(
    ...
    const chatId = match[1];
    const result = await this.userManagementService.rejectUser(
      chatId,
      msg.from?.id.toString()
    );
  ...
  private async handleUsersCommand(
    bot: TelegramBot,
    msg: TelegramBot.Message,
    config: MasterBotConfig
  ): Promise<void> {
  ...
  private async handleGroupsCommand(
    ...
    try {
      ...
      await bot.sendMessage(msg.chat.id, groupsInfo, {
        parse_mode: "Markdown",
      });
    } catch (error) {
      ...
      await bot.sendMessage(
        msg.chat.id,
        "❌ Ошибка получения информации о группах"
      );
    }
  }

  private async handleBotsCommand(
    bot: TelegramBot,
    msg: TelegramBot.Message,
    config: MasterBotConfig
  ): Promise<void> {
    ...
    try {
      ...
      await bot.sendMessage(
        msg.chat.id,
        "❌ Ошибка получения информации о ботах"
      );
    }
  }
