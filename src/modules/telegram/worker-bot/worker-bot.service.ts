...
export class WorkerBotService {
  ...
  private async handleUserMessage(bot: TelegramBot, msg: TelegramBot.Message, botId: string) {
    ...
    try {
      ...
      this.logger.debug(
        `handleUserMessage: chatType=${chatTypeInfo}, chatId=${chatIdInfo}, fromId=${fromIdInfo}, textLen=${textLen}, botId=${botId}`,
      );
    } catch {}
  ...
  private async handleRoleCommand(
    ...
    if (roleArg && (roleArg === 'user' || roleArg === 'admin')) {
      try {
        ...
        await this.rolesService.assignRoleToUser(String(user.id), botId, roleArg as RoleTypeEnum);
      ...
      await bot.sendMessage(msg.chat.id, 'Выберите роль для входа в веб-приложение:', {
        reply_markup: {
          inline_keyboard: [
            [
   ...
   */
  private async handleWebAppCommand(
    bot: TelegramBot,
    msg: TelegramBot.Message,
    userId: string,
    botId: string,
  ...
  private setupCommandHandlers(bot: TelegramBot, botId: string) {
    ...
    bot.onText(/\/start/, async (msg) => {
      try {
        if (!msg.from) return;
        const user = await this.usersService.findOrCreate(msg.from.id.toString(), {
    ...
    bot.on('callback_query', async (callbackQuery: TelegramBot.CallbackQuery) => {
      try {
        ...
        if (data.startsWith('role_')) {
          ...
          const user = await this.usersService.findOrCreate(telegramUserId, {
          ...
          this.logger.log(
            `Попытка обновления роли пользователя ${user.id} (Telegram ID: ${telegramUserId}) на ${role} для бота ${botId}`,
          );
    ...
    try {
      ...
      if (this.bots.has(token)) {
        ...
        if (existingBot) {
          ...
          try {
            ...
            this.logger.warn(
              `Не удалось корректно остановить старый экземпляр бота: ${e instanceof Error ? e.message : String(e)}`,
            );
          }
        }
      ...
      try {
        ...
        try {
          ...
          this.logger.warn(
            `Не удалось корректно закрыть тестовый экземпляр бота: ${e instanceof Error ? e.message : String(e)}`,
          );
        }
      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Неизвестная ошибка';
      ...
      try {
        ...
        bot.on('my_chat_member', async (update: TelegramBot.ChatMemberUpdated) => {
          try {
            ...
            const oldStatus = update.old_chat_member?.status || 'unknown';
            const newStatus = update.new_chat_member?.status || 'unknown';
            this.logger.log(
              `my_chat_member: chatType=${chat?.type}, chatId=${chat?.id}, title="${chat?.title || ''}", performerId=${performer?.id}, oldStatus=${oldStatus}, newStatus=${newStatus}`,
            );
            ...
            if (chat && (chat.type === 'group' || chat.type === 'supergroup')) {
              ...
              try {
                ...
                this.logger.log(
                  `Группа зарегистрирована через my_chat_member: ${chatId} (${chatTitle})`,
                );
              } catch (regErr) {
                const err = regErr instanceof Error ? regErr.message : String(regErr);
                this.logger.error(
                  `Ошибка регистрации группы через my_chat_member chatId=${chatId}: ${err}`,
                );
              }
            }
          } catch (e) {
        ...
        bot.on('chat_member', async (update: TelegramBot.ChatMemberUpdated) => {
          try {
            ...
            const newStatus = update.new_chat_member?.status || 'unknown';
            this.logger.log(
              `chat_member: chatType=${chat?.type}, chatId=${chat?.id}, title="${chat?.title || ''}", userId=${user?.id}, oldStatus=${oldStatus}, newStatus=${newStatus}`,
            );
          } catch (e) {
            const err = e instanceof Error ? e.message : String(e);
        ...
        try {
          ...
          const whUrl = whInfo.url || '';
          if (whUrl) {
            this.logger.warn(
              `Обнаружен активный webhook у бота ${name}: ${whUrl}. Удаляем webhook перед стартом polling...`,
            );
          ...
          this.logger.warn(
            `Не удалось получить/удалить webhook: ${whErr instanceof Error ? whErr.message : String(whErr)}`,
          );
        }
        ...
        this.logger.log(
          `Проверяем состояние бота ${name}: isBotWorking=${isBotWorking}, isPolling=${bot.isPolling()}`,
        );

        // Проверяем, запустился ли бот
        if (!isBotWorking && !bot.isPolling()) {
          throw new Error(
            `Бот ${name} не смог запуститься: polling_init не сработал и isPolling=false`,
          );
        }
        ...
        if (!isBotWorking && bot.isPolling()) {
          this.logger.warn(
            `Бот ${name}: polling_init не сработал, но опрос работает. Считаем бота активным.`,
          );
          isBotWorking = true;
        }
  ...
  async stopBot(token: string): Promise<boolean> {
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          `Не удалось принудительно закрыть бота: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
      ...
      this.logger.error(
        `Ошибка при остановке бота: ${error instanceof Error ? error.message : String(error)}`,
      );
      return false;
    }
  }
